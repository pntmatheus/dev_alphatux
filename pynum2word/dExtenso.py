...
class dExtenso():
    ...
    def to_currency(self, valor):
        if "." in str(valor):
            valor = Decimal(valor).quantize(Decimal('1.00'))
            a, b = str(valor).split(".")
            if str(a) == "0":
                return self.getExtenso(b) + " Centavos"
            else:
                return self.getExtenso(a) + " Reais e " + self.getExtenso(b) + " Centavos"
        else:
            return self.getExtenso(str(valor)) + " Reais"
